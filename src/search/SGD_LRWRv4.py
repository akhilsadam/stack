...
class SphericalProjector(nn.Module):
    ...
    def pretrain(self, x, steps: int = 1000):
        ...
        for i in range(steps):
            ...
            _str_1 = self.tok.token_embed.reverse(x_1, max_seq_len=self.tok.seq_len // 4)
            x_1 = self.tok.token_embed(_str_1)

            x_2 = torch.randn_like(x)
            _str_2 = self.tok.token_embed.reverse(x_2, max_seq_len=self.tok.seq_len // 4)
            x_2 = self.tok.token_embed(_str_2)

            alphas = torch.rand((x.shape[0],), device=x.device)
            _str_a = [_str_1a + " " + str(alpha.item()) + " * " + _str_2a + " " + str(1 - alpha.item()) + " *  +" for _str_1a, _str_2a, alpha in zip(_str_1, _str_2, alphas)]
            x_a = self.tok.token_embed(_str_a)

            e_1 = self.encode(x_1)
            e_2 = self.encode(x_2)
            e_a = alphas.unsqueeze(1) * e_1 + (1 - alphas).unsqueeze(1) * e_2
            loss_enc = F.mse_loss(e_a, self.encode(x_a))

            _str_b = [_str_1a + " " + _str_1b + " * " for _str_1a, _str_1b in zip(_str_1, _str_2)]
            x_b = self.tok.token_embed(_str_b)
            e_b = self.encode(x_b)
            loss_enc_2 = F.mse_loss(e_b, e_1 * e_2)

            loss_reco = F.mse_loss(self.decode(e_1), x_1)

            loss = loss_reco + loss_enc + loss_enc_2
...
class Search(nn.Module):
    ...
    def search(self, init_pde: str, target_pde: str | None = None):
        ...
        for step in tqdm(range(self.steps), desc="Flow Matching Search"):
            ...
            eps[0] = 0 # keep the first one as the current state
            x_cand = tok_p + self.noise_std * eps


            # --- FAST ONLINE FIT ---
            # Quick 3-step update to adapt projector to current population manifold
            # self.projector.train_on_fly(x_cand.detach())
            # -----------------------
            ...
            # 4. Form velocity target (x1_hat) via soft-min weighting of valid manifold states IN LATENT
            weights = torch.softmax(-pde_losses / self.tau, dim=0)
            clat = self.encoder(tok_p)
            lats = self.encoder(x1_rounded)

            lat_target = (weights.view(-1, 1) * lats).sum(dim=0, keepdim=True)
            # lat_target = F.normalize(lat_target, p=2, dim=-1) # renorm

            # 5. Euler Step along predicted velocity field: v = (x1_target - tok_p)
            velocity = lat_target - clat
            clat = clat + self.lr * velocity
            # renorm to keep it on sphere
            # clat = F.normalize(clat, p=2, dim=-1)
            tok_p = self.decoder(clat)

            tok_p, _ = self._round_trip(tok_p)
