...
class Search(nn.Module):
    ...
    def search(self, init_pde: str, target_pde: str | None = None):
        ...
        for step in tqdm(range(self.steps), desc="Flow Matching Search"):
            ...
            # want to improve this; it's a hard multimodal space, we want to relax
            # and work on a continuous space (move search to prior rather than posterior)
            # so we need a prior space z [low dim cond, EmbedOpt], and pushforward to tok_p that the search is smooth
            # so we need approximate loss wrt x to be linearized
            # EBM that generates x conditional on z ...
